...
sys_read(read_args_t *arg)
{
      ...
      dbg(DBG_PRINT,"(GRADING3B 1 SYS1)\n");
      int copyfromret=copy_from_user(&read_arg,arg,sizeof(read_args_t));
      void* buffer=page_alloc();
      int readret=do_read(read_arg.fd,buffer,read_arg.nbytes);
      int copytoret=copy_to_user(read_arg.buf,buffer,read_arg.nbytes);
      page_free(buffer);
      if(copytoret<0){
        dbg(DBG_PRINT,"(GRADING3B 1 SYS2)\n");
        curthr->kt_errno=copytoret;
        return -1;
      }
      return readret;
      */
...
sys_write(write_args_t *arg)
{
      /*  NOT_YET_IMPLEMENTED("VM: sys_write");*/
        ...
        int copyfromret=copy_from_user(&write_arg,arg,sizeof(write_args_t));
        dbg(DBG_PRINT,"(GRADING3B 1)\n");
        /*if (copyfromret < 0)
        {
          dbg(DBG_PRINT,"(GRADING3B 1 SYS4)\n");
       ...
       */
        ...
        /*void* buffer=page_alloc();
        */
        int writeret=do_write(write_arg.fd,arg->buf,write_arg.nbytes);
        /*int copytoret=copy_to_user(write_arg.buf,buffer,write_arg.nbytes);
        */
        /*page_free(buffer);
        */
        /*if(writeret<0){
          dbg(DBG_PRINT,"(GRADING3B 1 SYS5)\n");
          curthr->kt_errno=-writeret;
          return -1;
        }
        */
        return writeret;
...
sys_getdents(getdents_args_t *arg)
{
      ...
      /*getdents_args_t dents_arg;
      int copyfromret=copy_from_user(&dents_arg,arg,sizeof(getdents_args_t));
      dirent_t *buffer=(dirent_t*)kmalloc(sizeof(dirent_t));
      dbg(DBG_PRINT,"(GRADING3B 1 SYS6)\n");
      size_t count=0;
      --commented dirent_t *dirp=dents_arg.dirp;
      for(count=0;count<dents_arg.count / sizeof(dirent_t);count++){
        int dentret=do_getdent(dents_arg.fd,buffer);
        error handler to added later
        int copytoret=copy_to_user(dents_arg.dirp,buffer,sizeof(dirent_t));
        dents_arg.dirp++;  or +sizeof(dirent_t)?
      }
      kfree(buffer);
      int result=count*sizeof(dirent_t);
      return result;
      */
      return NULL;
}
