 ...
 */
 ...
 pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
 {
         ...
         int ret;
         /*if resident and busy,wait for unbusy and try again*/
         dbg(DBG_PRINT,"(GRADING3B 1)\n");

         if(pf==NULL){
           dbg(DBG_PRINT,"(GRADING3B 1)\n");

             /*Before allocating the new pframe, we check to
              see if we need to call pageoutd and wake it up if necessary.*/
        ...
        *result=pf;
        if(*result!=NULL){
          KASSERT(NULL != *result);
          dbg(DBG_PRINT,"(GRADING3A 1.a) get correct result\n");
 ...
 pframe_pin(pframe_t *pf)
 {
         ...
         dbg(DBG_PRINT,"(GRADING3A 1.b) pf not free\n");
         KASSERT(pf->pf_pincount >= 0);
         dbg(DBG_PRINT,"(GRADING3A 1.b) pin count legal\n");
         if(!pframe_is_pinned(pf)){
           dbg(DBG_PRINT,"(GRADING3B 1)\n");
           list_remove(&pf->pf_link);
 ...
 pframe_unpin(pframe_t *pf)
 {
         ...
         KASSERT(!pframe_is_free(pf));
         dbg(DBG_PRINT,"(GRADING3A 1.c) pf not free\n");
         KASSERT(pf->pf_pincount > 0);
         dbg(DBG_PRINT,"(GRADING3A 1.c) pincount > 0\n");
         pf->pf_pincount--;
         /*if reaches zero*/
         if(pf->pf_pincount==0){
           dbg(DBG_PRINT,"(GRADING3B 1)\n");
